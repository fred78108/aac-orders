...
class TestAdrFilesExist:
    ...
    def test_adr_file_exists(
        ...
        assert (adr_dir / filename).is_file(), (
            f"Missing ADR: {filename}"
        )

    def test_adr_index_readme_exists(
...
class TestAdrDecisionContent:
    ...
    def test_saga_adr_chose_choreography(
        self, adr_dir: pathlib.Path
    ) -> None:
        """ADR-0001 must confirm choreography was chosen."""
        src = (
            adr_dir / "0001-choreography-saga-pattern.md"
        ).read_text(encoding="utf-8")
        assert "choreography" in src.lower()

    def test_broker_adr_chose_rabbitmq(
        self, adr_dir: pathlib.Path
    ) -> None:
        """ADR-0002 must name RabbitMQ as the broker choice."""
        src = (
            adr_dir / "0002-rabbitmq-as-message-broker.md"
        ).read_text(encoding="utf-8")
    ...
    def test_database_adr_chose_per_service(
        ...
        src = (
            adr_dir / "0003-database-per-service.md"
        ).read_text(encoding="utf-8")
        assert "database per service" in src.lower()

    def test_gateway_adr_chose_nginx(
        self, adr_dir: pathlib.Path
    ) -> None:
        """ADR-0004 must name Nginx as the API gateway."""
        src = (
            adr_dir / "0004-nginx-api-gateway.md"
        ).read_text(encoding="utf-8")
        assert "Nginx" in src
    ...
    def test_language_adr_chose_python_312(
        ...
        src = (
            adr_dir / "0005-python-3-12-service-language.md"
        ).read_text(encoding="utf-8")
    ...
    def test_runtime_adr_chose_docker_compose(
        ...
        src = (
            adr_dir / "0006-docker-compose-local-runtime.md"
        ).read_text(encoding="utf-8")
...
class TestAdrCrossReferences:
    ...
    def test_saga_adr_links_to_broker_adr(
        self, adr_dir: pathlib.Path
    ) -> None:
        ...
        assert "0002" in src, (
            "ADR-0001 should link to ADR-0002"
        )

    def test_broker_adr_links_to_saga_adr(
        self, adr_dir: pathlib.Path
    ) -> None:
        ...
        assert "0001" in src, (
            "ADR-0002 should link to ADR-0001"
        )
    ...
    def test_database_adr_links_to_saga_adr(
        ...
        assert "0001" in src, (
            "ADR-0003 should link to ADR-0001"
        )
    ...
    def test_diagrams_adr_links_to_deployment_adr(
        ...
        assert "0006" in src, (
            "ADR-0007 should link to ADR-0006"
        )
